...
class TestContext(unittest.TestCase):
    ...
    def test_own_cue_near_the_value_assigns_own(self):
        ...
        self.assertEqual(self._match(_scope(), CLIENT_IP, text).role, scope.ROLE_OWN)
    ...
    def test_no_offsets_means_no_context_lookup(self):
        s = _scope()
        self.assertEqual(s.role_of("IP", ADVERSARY_IP).role, scope.ROLE_UNKNOWN)

    def test_custom_cues_replace_the_defaults(self):
        s = scope.Scope(cues={scope.ROLE_ADVERSARY: ["cavallo di troia"]})
...
class TestLoad(unittest.TestCase):
    ...
    def test_round_trip_of_lists_and_context(self):
        self.write({"own": {"IP": [CLIENT_NET]},
                    ...
                    "context": {"window": 10, "cues": {"adversary": ["beacon"]}}})
        s = self.load()
